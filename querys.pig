...
agg_big5 = FOREACH big5 GENERATE 
(EXT1-EXT2+EXT3-EXT4+EXT5-EXT6+EXT7-EXT8+EXT9-EXT10) AS EXT, 
(EST1-EST2+EST3-EST4+EST5+EST6+EST7+EST8+EST9+EST10) AS EST, 
(AGR2-AGR1-AGR3+AGR4-AGR5+AGR6-AGR7+AGR8+AGR9+AGR10) AS AGR,
(CSN1-CSN2+CSN3-CSN4+CSN5-CSN6+CSN7-CSN8+CSN9+CSN10) AS CSN,
(OPN1-OPN2+OPN3-OPN4+OPN5-OPN6+OPN7+OPN8+OPN9+OPN10) AS OPN, country AS country, 
lat_appx_lots_of_err as latitud,
long_appx_lots_of_err AS longitud;
...
agg_country_big5 = FOREACH by_country GENERATE 
FLATTEN(ROUND(AVG(agg_big5.EXT))) AS EXT,
FLATTEN(ROUND(AVG(agg_big5.EST))) AS EST,
FLATTEN(ROUND(AVG(agg_big5.AGR))) AS AGR,
FLATTEN(ROUND(AVG(agg_big5.CSN))) AS CSN,
FLATTEN(ROUND(AVG(agg_big5.OPN))) AS OPN,
FLATTEN(COUNT($1)) AS count_res, 
FLATTEN(group) as country;
